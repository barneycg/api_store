...
function displayUsers(){
	global $database;
	$sql="select forum_name,toon_name,corp from users,toons where toons.users_id = users.id order by forum_name";
	...
	foreach (array_keys($mlist) as $member) {
		...
		foreach ($mlist[$member] as $toon) {
			$message .= "<td header=\"tname\">$toon[0]<br><span style=\"color:green\">$toon[1]</span></td>";
		}
		$message .= "</tr>\n";
	}
