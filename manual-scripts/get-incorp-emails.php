...
$result97 = $database->query($sql97);//$result97 = mysql_query($sql97,$conn) or die(mysql_error());
...
while ($row97=mysql_fetch_array($result97)){
	...
	$result96 = $database->query($sql96);//$result96 = mysql_query($sql96,$conn) or die(mysql_error()); 
	$row96=mysql_fetch_array($result96);
	$count=$row96['count'];
	$sql95 = "UPDATE users SET toon_count=$count where id=$users_id";
	$result95 = $database->query($sql95);//$result95 = mysql_query($sql95,$conn) or die(mysql_error());
}

$sql98 = "SELECT email FROM users WHERE toon_count>0 and poss_spy!=1 order by email";
$result98 = $database->query($sql98);//$result98 = mysql_query($sql98,$conn) or die(mysql_error());
